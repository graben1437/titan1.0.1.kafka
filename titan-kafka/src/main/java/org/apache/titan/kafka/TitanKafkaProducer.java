...
public class TitanKafkaProducer
{
    ...
    public TitanKafkaProducer(final String cfgProps, final String topic)
    {
        ...
        {
            System.out.println("failed to create producer: " + e.toString());
            // failed
            state = 0;
        }
    }
    ...
    public void publish(final String transmissionString)
    {
        ...
        LOGGER.debug(methodName + " begin: " + transmissionString);

        try
        {
            if (transmissionString.trim().length() > 0)
            {
                // doesn't matter the value for pKey because we don't use this key to partition
                // we use a simple round robin partitioner configured in the properties file
                // that gets passed the number of partitions (kafka does this for us when we publish)
                String pKey = "default";
    ...
    public static void main(String[] args)
    {
        ...
        {
            System.out.println("TitanKafkaProducer ending on error.");
        }
