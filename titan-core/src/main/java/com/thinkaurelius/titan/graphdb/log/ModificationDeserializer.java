    ...
    public static InternalRelation parseRelation(TransactionLogHeader.Modification modification, StandardTitanTx tx) {
        ...
        if (type.isPropertyKey()) {
            if (state==Change.REMOVED) {
                ...
                rel = new StandardVertexProperty(relCache.relationId,(PropertyKey)type,outVertex,relCache.getValue(), ElementLifeCycle.ChangeLog);
            } else {
            ...
            if (state==Change.REMOVED) {
                ...
                rel = new StandardEdge(relCache.relationId, (EdgeLabel) type, outVertex, otherVertex,ElementLifeCycle.ChangeLog);
            } else {
        ...
        if (state==Change.REMOVED && relCache.hasProperties()) { //copy over properties
            for (LongObjectCursor<Object> entry : relCache) {
                ...
                {
                    out.println("setting properties on rel:" + entry.key + " " + entry.value);
                }
                catch (IOException e)
                {
                    e.printStackTrace();
                }
                // END TEMPORARY KAFKA DEBUG
            }
        }
        return rel;
    }
