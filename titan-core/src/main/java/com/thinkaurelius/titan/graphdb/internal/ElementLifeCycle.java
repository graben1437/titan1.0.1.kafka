...
public class ElementLifeCycle {
    ...
    // KAFKA CHANGE
    public enum Event {REMOVED, REMOVED_RELATION, ADDED_RELATION, UPDATE, CHANGELOG }
    ...
    /**
     * KAFKA PRODUCER CHANGE
     */
     public final static byte ChangeLog = 7;
    ...
    // KAFKA CHANGE
    public static boolean isChangeLog(byte lifecycle)  {
        return lifecycle>=New && lifecycle<=ChangeLog;
    }

    /**
     * This method enforces a "state machine" of transitions
     * for an object in Titan.  If this "event" just happened
     * then what should the lifecycle state be..and what was
     * the previous state.
     *
     * KAFKA comment - not sure this method needs to change
     * to support the change log setting
     */
    public static final byte update(final byte lifecycle, final Event event) {
        ...
        else if (event==Event.UPDATE)  {return Modified;}
        else if (event==Event.CHANGELOG) {return ChangeLog;}  //KAFKA CHANGE
